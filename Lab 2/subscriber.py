...
def on_message(client, userdata, message):
    ...
    #message = struct.unpack("f", message.payload)
    message = int(message.payload.decode())
    print(f"Recieved Message Count: {message}")
...
while True: # perhaps add a stopping condition using some break or something.
    print(f"Publishing Message Count: {message_count}")
    client.publish("ece180d/test/team5", int(message_count), qos=1)
    time.sleep(4)
